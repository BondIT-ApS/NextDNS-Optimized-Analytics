...
if PROFILE_IDS:
    ...
    logger.info(f"🧱 Configured {len(profile_ids)} profile(s): {', '.join(profile_ids)}")
else:
...
if not API_KEY or not profile_ids:
    ...
    logger.info(f"✅ NextDNS API configured with API key and {len(profile_ids)} profile(s)")
    ...
    def fetch_logs():
        ...
        logger.info(f"🔄 Starting multi-profile NextDNS log fetch (Database has {initial_count:,} records)")
        ...
        for profile_id in profile_ids:
            try:
                ...
                if last_fetch:
                    ...
                    logger.info(f"📅 Profile {profile_id}: incremental fetch from {from_time}")
                else:
                ...
                if response.status_code == 200:
                    ...
                    for log in logs:
                        ...
                        if record_id:
                            ...
                            logger.warning(f"⚠️  Profile {profile_id}: failed to process log for domain: {log.get('domain')}")
                    ...
                    logger.info(f"💾 Profile {profile_id}: {profile_added} NEW records added, {profile_skipped} duplicates skipped")
                    ...
                    logger.error(f"⚠️  Profile {profile_id}: API returned status {response.status_code}: {response.text}")
                    failed_profiles += 1
        ...
        logger.info(f"📊 Total: {total_added} NEW records added, {total_skipped} duplicates skipped")
        logger.info(f"📊 Profiles: {successful_profiles} successful, {failed_profiles} failed")
        logger.info(f"📊 Database now has {final_count:,} total records (+{total_added:,} new this fetch)")
